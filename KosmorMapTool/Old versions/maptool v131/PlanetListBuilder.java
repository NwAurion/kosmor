...
public class PlanetListBuilder {
	 ...
	 */
	public static Planet generateSingleWarplanet(int currentPosition, boolean is_warplanet) {
		...
		Planet warplanet = new Planet(name, xPos, yPos, color, is_warplanet);
	 ...
	 */
	public static Planet generateSinglePlanet(int currentPosition, boolean is_warplanet) {
		...
		Planet planet = new Planet(name, xPos, yPos, color, is_warplanet);
		return planet;
	}
	...
	public static LinkedList<Planet>[] generateAllPlanets() {
		...
		for (int currentPosition = planetCount; currentPosition <= totalPlanets; currentPosition++) {
			is_warplanet = true;
			Planet warplanet = generateSingleWarplanet(currentPosition, is_warplanet);
			warplanetList.add(warplanet);
		}

		for (int currentPosition = 0; currentPosition <= planetCount; currentPosition++) {
			is_warplanet = false;
			Planet planet = generateSinglePlanet(currentPosition, is_warplanet);
