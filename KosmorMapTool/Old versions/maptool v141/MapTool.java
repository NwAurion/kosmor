...
public class MapTool {
	...
	private static Boolean writeWarplanets = false;
	private static Boolean writePlanets = false;
	private static Boolean writeNeutral = false;

	/**
	 * Reads the kosmor svg map (which it downloads first if necessary) and creates a .html
	 * with the info read from the .svg. Parameters determine what exactly will be written
	 * to the .html
	...
	args = new String[]{"-login Aurion", "-password Ihn1kmpw", "-planets"};
		// Reading the String[] args parameter lists, to find out which ones where used
		...
		if (!svgMap.exists()){
			System.out.println("Connecting to Kosmor.com to download the map"); // Status message #1 Downloading
			HTTPClientForKosmor.doConnect(player_login, player_password);
			System.out.println("Succesfully downloaded the map"); // Status message #2 Downloading
		}
			
		System.out.println("Please stand by while it's working."); // Status message #1 Map Tool
		System.out.println("It should not take longer then a minute"); // Status message #2 Map Tool
		...
		if(argsString.contains("-date")){
			...
			try {
				...
				HTMLBuilder.writeHTML(writeWarplanets, writePlanets, writeNeutral, date);
		...
		else {	// If no -date parameter was used, the current system date will be used
			startTime = System.currentTimeMillis();
			HTMLBuilder.writeHTML(writeWarplanets, writePlanets, writeNeutral, new Date());
			endTime = System.currentTimeMillis();
		}
		System.out.println("Finished. It took "+(endTime-startTime)/1000+" seconds"); // Status message #3 Map Tool
	}
	...
	private static String[] readArgumentList(String argsString) {
		...
		if (argsString.contains("-login")) {
			player_login = argsString.substring(argsString.indexOf("-login")+7, argsString.indexOf("-", argsString.indexOf("-login")+1));
			player_login = player_login.substring(0, player_login.length()-2);
			System.out.println(player_login);
		}
		if (argsString.contains("-password")) {
			player_password = argsString.substring(argsString.indexOf("-password")+10, argsString.indexOf("-", argsString.indexOf("-password")+1));
			player_password = player_password.substring(0, player_password.length()-2);
			System.out.println(player_password);
		}
